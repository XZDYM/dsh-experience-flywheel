...
import { createClaims, addClaim, pruneClaims, listClaims, removeClaims, claimsSize, extractClaimedPaths } from "./claims.js";

// ─── Message-id factory (mirrors @deepseek-ai/dsh-llm `createUserMessage`) ───
// Required because session events of type user/message / assistant/message / tool/result
// must carry data.message.id (dsh-session assertMessageEventShape @ lib/index.js:1248).
// Plugins cannot import @deepseek-ai/dsh-llm directly (zero-dep contract) so we replicate
// the factory here. KEEP IN SYNC with dsh-llm/lib/types/message.js createUserMessage.
/** Brand a message identifier (no-op; identity is preserved). */
function MessageId(id) { return id; }
/** Deep-freeze a value in place; iterative traversal guards cycles; AbortSignal skipped. */
...
/** Detach and deep-freeze a message that already carries its identity. */
function freezeMessage(message) { return deepFreeze(structuredClone(message)); }
/** Create one identified message and freeze it (mints UUID). */
function createMessage(input) {
  return freezeMessage({ ...input, id: MessageId(crypto.randomUUID()) });
}
/** Create one identified user-role message and freeze it (mints UUID). */
 ...
 *  Must carry an `id` (UUID) so dsh-session assertMessageEventShape (data.message.id required)
 *  passes; bare messages freeze the entire session load. Local createUserMessage mirrors
 *  @deepseek-ai/dsh-llm `createUserMessage` to avoid a hard dependency on dsh-llm. */
 ...
 *  F3: 白名单+形状校验已抽到 claims.js（probe 可单测）；此处只留转发。 */
function claimedPaths(exec) {
  return extractClaimedPaths(exec);
}
